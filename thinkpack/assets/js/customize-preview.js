/* global thinkpackBgColors, thinkpackPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( thinkpackBgColors ).forEach( function( context ) {
					thinkpackGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function thinkpackGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'thinkpack-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#thinkpack-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( thinkpackPreviewEls[ context ] ) {
				_.each( thinkpackPreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		thinkpackGenerateColorA11yPreviewStyles( 'content' );
		thinkpackGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
